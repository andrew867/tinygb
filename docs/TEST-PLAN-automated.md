...
| AUTO-DATA-001 | `test_settings`: defaults; format; round trip of all five keys; unknown keys ignored; CRLF tolerated; a misspelt palette is the default | REQ-DATA-030..031 |
| AUTO-UI-003 | `test_menu_raw`: opens on the shelf; a tap on a cartridge plays it with its path and a clean title; a drag does not choose; Settings, Palette and About open; a toggle flips and reports; a palette choice reports and goes back; the chevron and an edge swipe go back; the pause page's rows renumber without a state; back from pause is Resume; the last row leaves for the shelf; a press already down at open does nothing; Vol Down moves the highlight with a delayed repeat; the Resume row leads the shelf when a state exists | REQ-ROS-031, -032, -035, REQ-DATA-023 |
