...
| AUTO-AUD-001 (new) | `test_aud_slots` over the fake descriptor writer: FIFO reclaim only after `+0x64` seen 1 then 0; seal at 1470 frames; link is a single store to the tail's `+0x54`; first slot is played, later ones only linked | REQ-AUD-010..013 |
| AUTO-AUD-002 (new) | `test_aud_duration`: exact ms only when `frames*1000 % out_rate == 0` and the inverse round-trips, else 0; 1470 @ 22050 -> 0 (not exact), 2205 @ 22050 -> 100 | REQ-AUD-010 |
| AUTO-AUD-003 (new) | `test_aud_silence`: pause zeros queued slots, ramps the head from cursor + 250 ms over 80 ms, unlinks the chain, links the quiet block to itself; resume clears the quiet block's next and fades the next seal | REQ-AUD-020..022 |
