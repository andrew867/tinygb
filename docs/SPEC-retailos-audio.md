...
- **REQ-AUD-001**: The sink (`platform/tg_audq.h`) shall present the emulator with a queue of fixed-length slots. Constants, all in one place: `TG_AUDQ_RATE 22050`, `TG_AUDQ_SLOT_FRAMES 1323` (60 ms, 3 x 441: a multiple of 441 is what makes the descriptor's duration field convert exactly on a 22050 or 44100 mixer; the 1470 first proposed did not), `TG_AUDQ_SLOTS 6` (360 ms of buffer), `TG_AUDQ_TARGET_LEAD 2` slots queued beyond the one sounding (120 to 180 ms of latency).
- **REQ-AUD-002**: Slot and silence PCM buffers shall be one static pool in `.bss` (`TG_AUDQ_POOL_SAMPLES`, 63 KB), handed to `tg_audq_init` once at launch and never freed. Static rather than `hb_os_alloc`: it costs nothing in the file, it cannot leak per launch, and the arena outlives any voice by the seconds it takes to launch anything again. Descriptors (`uint8_t desc[0x80]`) live in the queue.
- **REQ-AUD-003**: `tg_audq_queued_frames()` shall return the number of PCM frames not yet played, computed from slots in the QUEUED state plus the unplayed remainder of the sounding one (wall-clock estimate from `now_ms` since that slot started, clamped) plus what is in the block being built. `tg_audq_frames_wanted()` is the target `(TARGET_LEAD + 1) * SLOT_FRAMES` minus that; the front end runs an emulated frame while it is positive, at most two per tick.
- **REQ-AUD-004**: Everything the OS does shall be behind `tg_audq_ops` (constructor, chain store, play, the voice's rate and counter, the clock), so the queue's state machine runs on the host against a fake audio task. `platform/tg_audio_hb.c` binds the real addresses.

### Feeding

- **REQ-AUD-010**: The front end shall push audio in emulated-frame units (369 or 370 frames) with `tg_audq_push`, which fills the current block and spills into the next free one; when a block reaches `TG_AUDQ_SLOT_FRAMES` it shall be sealed: descriptor fields written, `+0x34` set to the exact millisecond duration only when `frames * 1000 % out_rate == 0` and the value round-trips (else 0), then linked to the tail with a single aligned 32-bit store to the tail's `+0x54`. A push with no free block drops the frames and counts them; it never blocks.
...
- **REQ-AUD-020**: Pause (menu open) shall unlink every queued slot, zero the ones behind the head, and in the sounding slot apply a 40 ms linear ramp from `play_cursor + 40 ms` then zeros (with 60 ms blocks the sounding one usually plays out and it is the ones behind it that go quiet). Then chain a self-linked 320 ms silent block so the voice stays alive and resume is a ramp, not a cold start.
- **REQ-AUD-021**: Resume shall leave the silent block looping until the next block is sealed, which is chained from it (the silence's `+0x54` pointed at the new block, no second voice) and fades in over its first 40 ms.
- **REQ-AUD-022**: Cartridge change (`tg_audq_stop`) shall silence as in pause and leave the silent block looping; the queue is initialised once per launch, never per cartridge, so a voice is never started while another loops silence.
