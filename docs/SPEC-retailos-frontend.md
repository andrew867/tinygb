...
- **REQ-ROS-050**: The build shall be freestanding: no `malloc`, no `printf` family, no `qsort`, no `<math.h>`. The portable modules reach the OS only through `tg_sys.h` (whole-file read and write, clock, log) and `tg_util.h` (the string functions the SDK's shim lacks, including `strrchr`); `tg_sys_hb.c` and `tg_roms_hb.c` are the RetailOS backends, and `tg_sys_hb.c` also provides `abort`, which the vendored core names in a branch the compiler never takes.
...
- **REQ-ROS-071**: The root `Makefile` shall set `HB_ARCH_FLAGS := -mcpu=cortex-a5 -mthumb -mfpu=vfpv4` (the part, per OQ-008; `hb_app.mk` reads its target from that variable since Phase 3) and `EXTRA_CFLAGS` with `-O2`, `-DAUDIO_SAMPLE_RATE=22050 -DMINIGB_APU_AUDIO_FORMAT_S16SYS=1`, `-DTG_SAVE_CHECK_MS=2000`, and the vendor warning suppressions, all before the include. No `time.h` shim is needed: the cross compiler's own headers satisfy the vendored core's include.
