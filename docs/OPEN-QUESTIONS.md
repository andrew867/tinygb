...
| OQ-008 | RetailOS compiler flags | RetailOS build | Medium | Phase 3 | Decided and in effect: `-O2 -mcpu=cortex-a5 -mthumb -mfpu=vfpv4`. `hb_app.mk` in the NanoApps fork reads its target from `HB_ARCH_FLAGS` (default unchanged, other apps unmoved, verified with Paint and Clock); TinyGB sets it and the whole link, libgcc included, follows. `-fjump-tables` is still to be measured on the device. |
