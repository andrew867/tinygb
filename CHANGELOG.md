...
- The RetailOS menu (Phase 5): `ui/tg_menu_raw.c`, a hand-drawn menu in the nano's own shape (title bar with back chevron, rows with chevrons, toggles and check marks, theme colours from NanoApps' prefs, tap / drag / edge-swipe, Vol Up/Down highlight); the pause page behind the pill with save and load states; Settings with palette, scaling, tilt and a counters overlay; About with the build stamp; a Resume row on the shelf for the last game. `platform/tg_settings.c` persists the settings as `key=value`. The idle-reset poke keeps the panel lit while a game runs. Twenty-five host checks tap through the menu and round-trip the settings.
- Sound on RetailOS (Phase 4): `tg_audq`, a queue of 60 ms PCM blocks the OS mixer plays as a chain, with the queue's depth deciding how many emulated frames a tick runs; `tg_audio_hb` binds it to the firmware; pause, resume and cartridge change go quiet in place; the Music player is re-paused if RetailOS starts it under the game. `make TG_TONE=1` builds a 440 Hz test tone in place of the APU. Thirty host checks drive the queue against a fake audio task.
